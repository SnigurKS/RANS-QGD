...
{
    public class AlgGenerator : MeshGenerator
    {
        ...
        //
        public AlgGenerator() { }
        public AlgGenerator(AlgParameter p) 
        {
            ChangeParameters(p);
        }
        public void ChangeParameters(AlgParameter p)
        {
            ...
            for (int i = 0; i < NY; i++)
                fEta[i] = 1 - 2 * (float)Eta[i];
            rangePartitioner = Partitioner.Create(0, NX);
        }
        protected override void GenerateCoords(SimpleArea sArea)
        {
        ...
        private void AllocateHostMemory(bool bPinGenericMemory, ref double[] pp_a, ref CudaPageLockedHostMemory<double> pp_Aligned_a, int nbytes)
        {
            ...
            pp_Aligned_a = new CudaPageLockedHostMemory<double>(nbytes / sizeof(double));
            pp_a = new double[nbytes / sizeof(double)];
        }
        //
        /// <summary>
        /// Генерация сетки средствами OpenCL
        /// </summary>
        ...
        double[][] OCL_AlgGrid()
        {
            ...
            if (Cuda)
            {
                ...
                if (IntPtr.Size == 8)
                    ...
                    resName = "AlgGrid_x64.ptx";
                string resNamespace = "MeshLibrary";
        ...
        double[] OCL_AlgGrid(int k)
        {
            if ((k == 1) || (k == 0))
            {
                ...
                {
                    // твой код вызова ядра на Cuda
                    // результат возвращается в качестве одномерного массива
                    //
                    return null;
                }
                else if(OpenCL)
                {
                    try
                    {
                        ...
                        string s = AppDomain.CurrentDomain.BaseDirectory;
                        //
                        System.IO.StreamReader streamReader = new System.IO.StreamReader("..\\..\\..\\MeshLibrary\\OpenCL\\AlgGrid.cl");
                        string clSource = streamReader.ReadToEnd();
                        streamReader.Close();
                        ComputeProgram program = new ComputeProgram(context, clSource);
                        //
