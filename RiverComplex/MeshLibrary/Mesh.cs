...
namespace MeshLibrary
{
    ...
    public class Mesh
    {
        ...
        public void TriangleGeometryCalculation(bool structureChanged = true)
        {
            ...
            Xcr = new double[CountElems][];
            Ycr = new double[CountElems][];
        ...
        public void MakeWallFuncStructure(bool surf_flag)
        {
            if (!surf_flag)
            {
                ...
                TWallDistance[0] = Math.Sqrt((Y[LeftKnots[0]] - Y[LeftKnots[1]]) * (Y[LeftKnots[0]] - Y[LeftKnots[1]]) + (X[LeftKnots[0]] - X[LeftKnots[1]]) * (X[LeftKnots[0]] - X[LeftKnots[1]]));
                TWallDistance[CountTop - 1] = Math.Sqrt((Y[TopKnots[0]] - Y[TopKnots[0] - 1]) * (Y[TopKnots[0]] - Y[TopKnots[0] - 1]) + (X[TopKnots[0]] - X[TopKnots[0] - 1]) * (X[TopKnots[0]] - X[TopKnots[0] - 1]));
            ...
            BWallDistance[CountBottom - 1] = Math.Sqrt((Y[RightKnots[1]] - Y[RightKnots[0]]) * (Y[RightKnots[1]] - Y[RightKnots[0]]) + (X[RightKnots[1]] - X[RightKnots[0]]) * (X[RightKnots[1]] - X[RightKnots[0]]));
        }
        /// <summary>
        ...
        public double GetNormalDistanceBottom(int p0)
        {
            ...
            double l = Ym - Y[(p0 / CountLeft) * CountLeft];
        ...
        //
        internal void TransportCVstructure(Mesh prevMesh)
        {
            if (prevMesh != null)
            {
