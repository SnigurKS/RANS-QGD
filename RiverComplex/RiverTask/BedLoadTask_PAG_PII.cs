...
namespace RiverTaskLibrary
{
    ...
    /// </summary>
    public class BedLoadTask_PAG_PII : BaseBedLoadTask
    {
        double s;
        public override string Name
        {
            get
            {
                return "Модель c придонным напором 2014";
            }
        }
        /// <summary>
        /// Конструктор 
        /// </summary>
        public BedLoadTask_PAG_PII()
            : base() { }
            ...
            switch (BCBed.Inlet)
            {
                ...
                case TypeBoundaryCondition.Neumann_boundary_conditions:
                    ...
                    //{
                        ...
                        //dz = dx / B * (q_in / qA - A + C * (ps[N] - ps[N - 1])); ////----????? N ????
        ...
        //
        public override double CalculateRate(double Tau, double J, double dp)
        {
        ...
        /// </summary>
        public override double[] Solve(double[] tau, double[] P, double[] Q0, double[] Q1)
        {
            try
            {
                double maxerr = 0;
                ...
                // Даавление в узлах Zeta,  Zeta0
                for (int j = 1; j < N; j++)
                    ps[j] = 0.5 * (P[j] + P[j - 1]) / (rhoW * g);
                // линейная интерполяция плоха!!! нужна квадратичная
                ps[0] = (2 * P[1] - P[2]) / (rhoW * g);
                ps[N] = (2 * P[N - 1] - P[N - 2]) / (rhoW * g);
                //
                for (int i = 0; i < N; i++)
                {
                    // расчет коэффициентов 
